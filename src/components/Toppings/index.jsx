...
const Toppings = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:4060/toppings")
  ...
  return (
    <div>
      ...
      <h3>Soslar Ücreti <span data-testid="total" className="text-success">{basket.length * 3}</span>₺</h3>

      <div className="row p-3 mt-4 gap-3">
        {data.map((i) => {
          const found = basket.find((item) => item.id === i.id)
          return (<div data-testid="card" onClick={() => handleChange(i)} className={`top-card col ${found ? "active" : ""}`} key={i.id}>
            <div>
